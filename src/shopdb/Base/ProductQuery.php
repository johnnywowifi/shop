 ...
 * @method     ChildProduct findOneByProductname(string $productname) Return the first ChildProduct filtered by the productname column
...
abstract class ProductQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, productname, description, price, category_id FROM product WHERE id = :p0';
        try {
     ...
     * $query->filterByProductname('%fooValue%'); // WHERE productname LIKE '%fooValue%'
     * </code>
     *
